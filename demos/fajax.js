...
(function($, FAjax){
	...
	var statuses = ['Active','Disabled','Suspended'];
	...
	FAjax.defaults = {
		delay: 1500,
		columns: [
			{ name: 'id', title: 'ID', type: 'number', style: { width: 80, maxWidth: 80 } },
			{ name: 'firstName', title: 'First Name', sorted: true, direction: 'ASC' },
			{ name: 'lastName', title: 'Last Name' },
			{ name: 'something', title: 'Something', visible: false },
			{ name: 'jobTitle', title: 'Job Title', breakpoints: 'sm', style: { maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', wordBreak: 'keep-all', whiteSpace: 'nowrap' } },
			{ name: 'started', title: 'Started', type: 'date', breakpoints: 'md', sortable: false, formatString: 'MMM YYYY' },
			{ name: 'dob', title: 'DOB', type: 'date', breakpoints: 'md', formatString: 'DD MMM YYYY' },
			{ name: 'status', title: 'Status', filterable: false }
		],
		rows: FAjax.createRows(100000),
		sorters: {
			text: function (a, b) {
	...
	FAjax.getColumn = function(name){
		for (var i = 0, len = FAjax.defaults.columns.length; i < len; i++){
			if (FAjax.defaults.columns[i].name == name) return FAjax.defaults.columns[i];
		}
		return {};
	};
	...
	FAjax.writeRows = function(){
		for (var i = 0; i < FAjax.defaults.rows.length; i++){
			var data = FAjax.defaults.rows[i],
				row = '<tr>';
			row += '<td style="color: red;">' + data.id + '</td>';
			row += '<td>' + data.firstName + '</td>';
			row += '<td>' + data.lastName + '</td>';
			row += '<td>' + data.something + '</td>';
			row += '<td>' + data.jobTitle + '</td>';
			row += '<td data-value="' + data.started + '"></td>';
			row += '<td data-value="' + data.dob + '"></td>';
		...
		return $.Deferred(function(d){
			setTimeout(function(){
				...
				if (data.currentPage && data.pageSize){
					...
					result.pageCount = Math.ceil(total / data.pageSize);
				}
				result.rows = rows;
				d.resolve(result);
